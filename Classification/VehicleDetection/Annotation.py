...
print("Creating image list... (could take a while)")
if PATH == None or os.path.exists(PATH) == False:
    print("PATH does not exist, exiting...")
    exit()
for i, file in enumerate(os.listdir(f"{PATH}")):
    if file.endswith(".png") and os.path.exists(os.path.join(f"{PATH}", file.replace(".png", ".txt"))) == False:
...
while True:
    ...
    try:
        image_resized = cv2.resize(image, (round(frame_width * 0.6), frame_height))
    ...
    for i in range(len(CLASSES) + 1):
        if i != len(CLASSES):
            ...
            button_delete_pressed, button_delete_hovered = make_button(text="Delete",
                                                                       x1=0.605*frame_width,
                                                                       y1=((i / (len(CLASSES) + 1) + 0.01) * 0.99)*frame_height,
                                                                       x2=0.7025*frame_width,
                                                                       y2=(((i + 1) / (len(CLASSES) + 1)) * 0.99)*frame_height,
                                                                       round_corners=30,
                                                                       ...
                                                                       width_scale=0.95,
                                                                       height_scale=0.3)
            button_continue_pressed, button_continue_hovered = make_button(text=f"Continue (Key: {Continue_Key.upper()})",
                                                                           x1=0.7075*frame_width,
                                                                           y1=((i / (len(CLASSES) + 1) + 0.01) * 0.99)*frame_height,
                                                                           x2=0.995*frame_width,
                                                                           y2=(((i + 1) / (len(CLASSES) + 1)) * 0.99)*frame_height,
                                                                           round_corners=30,
    ...
    if button_continue_pressed:
        text = f"{selected_classes}"
        text = text.replace("[", "").replace("]", "").replace("'", "").replace(" ", "")
        with open(os.path.join(f"{PATH}/{file.replace('.png', '.txt')}"), 'w') as f:
            f.truncate(0)
            f.write(text)
            f.close()
        annotation_counter += 1
